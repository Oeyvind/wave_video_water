...
def draw_status_hud(
    ...
    profile_lines = [
        ...
        f"waves   {profile_stats.get('wave_shapes_ms', 0.0):.2f} | {profile_stats.get('wave_shapes_pct', 0.0):.1f}%",
    ...
    legend_lines = [
        ...
        f"[O] slit+fft overlay: {'on' if switches['slit_fft_overlay'] else 'off'}",
        ...
        f"[Z] wave shapes: {'on' if switches['wave_shapes_overlay'] else 'off'}",
        f"[[/]] wave conf: {switches['wave_shape_conf_label']}",
        f"[5]/[6] wave min area: {switches['wave_shape_min_area_label']}",
        f"[7]/[8] wave max area: {switches['wave_shape_max_area_label']}",
        (
            "[,]/[.] order center: "
...
def draw_wave_shapes_overlay(frame, wave_shapes_data):
    if not isinstance(wave_shapes_data, dict):
        return frame
    detections = wave_shapes_data.get("detections", [])
    if not detections:
        return frame

    primary_wave = wave_shapes_data.get("primary_wave") if isinstance(wave_shapes_data, dict) else None
    primary_id = int(primary_wave.get("id", -1)) if isinstance(primary_wave, dict) else -1
    predicted_center = primary_wave.get("predicted_center") if isinstance(primary_wave, dict) else None
    gate_px = float(primary_wave.get("gate_px", 0.0)) if isinstance(primary_wave, dict) else 0.0

    out = frame.copy()
    for det in detections:
        x, y, w, h = [int(v) for v in det.get("bbox", (0, 0, 0, 0))]
        cx, cy = det.get("center", (0.0, 0.0))
        conf = float(det.get("confidence", 0.0))
        did = int(det.get("id", -1))
        is_primary = did == primary_id

        box_col = (0, 0, 255) if is_primary else (0, 235, 255)
        ctr_col = (0, 0, 255) if is_primary else (20, 245, 100)

        cv2.rectangle(out, (x, y), (x + w, y + h), box_col, 2)
        cv2.circle(out, (int(round(cx)), int(round(cy))), 4, ctr_col, -1, cv2.LINE_AA)
        cv2.putText(
            out,
            f"#{did} c={conf:.2f}{' P' if is_primary else ''}",
            (x, max(12, y - 6)),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.42,
            box_col,
    ...
    if predicted_center is not None:
        ...
        cv2.circle(out, (px, py), 6, (255, 120, 30), 1, cv2.LINE_AA)
        if gate_px > 1.0:
            cv2.circle(out, (px, py), int(round(gate_px)), (255, 120, 30), 1, cv2.LINE_AA)

    cnt = int(wave_shapes_data.get("count", len(detections)))
    lock_state = str(primary_wave.get("state", "-")) if isinstance(primary_wave, dict) else "-"
    draw_text_block(
        out,
        [f"waves: {cnt}", f"primary: {primary_id if primary_id >= 0 else '-'} {lock_state}"],
        10,
        10,
        anchor="top_left",
        font_scale=0.5,
...
def make_display_frame(
    ...
    if wave_shapes_overlay:
        out = draw_wave_shapes_overlay(out, analysis.get("wave_shapes_data"))
...
def main():
    ...
    show_slit_fft_overlay = True
    show_wave_shapes_overlay = False
    ...
    while True:
        ...
        analysis["slit_fft_data"] = _compute_center_slit_fft_data(analysis.get("preprocess_display"))
        ...
        switches = {
            ...
            "slit_fft_overlay": show_slit_fft_overlay,
            "wave_shapes_overlay": show_wave_shapes_overlay,
            ...
            "pre_blur_label": ["off", "small", "large", "xlarge"][max(0, min(3, analyzer.blur_mode))],
            ...
            "wave_shape_conf_label": f"{float(analyzer.wave_shape_min_confidence):.2f}",
            "wave_shape_min_area_label": f"{float(analyzer.wave_shape_min_area_px):.0f}px",
            "wave_shape_max_area_label": f"{float(analyzer.wave_shape_max_area_px):.0f}px",
        }
        ...
        if _do_render:
            display = make_display_frame(
                ...
                show_slit_fft_overlay,
                show_wave_shapes_overlay,
        ...
        profile_windows["wave_shapes_ms"].append(analysis["timings"].get("wave_shapes_ms", 0.0))
        ...
        if loop_end >= next_profile_update_t:
            ...
            avg_profile_pct = {
                ...
                "wave_shapes_pct": avg_profile_ms["wave_shapes_ms"] / total_for_pct * 100.0,
        ...
        if key == ord("o"):
            show_slit_fft_overlay = not show_slit_fft_overlay
        if key == ord("c"):
        ...
        if key == ord("z"):
            show_wave_shapes_overlay = not show_wave_shapes_overlay
        if key == ord("["):
            analyzer.set_wave_shape_params(min_confidence=analyzer.wave_shape_min_confidence - 0.02)
        if key == ord("]"):
            analyzer.set_wave_shape_params(min_confidence=analyzer.wave_shape_min_confidence + 0.02)
        if key == ord("5"):
            analyzer.set_wave_shape_params(min_area_px=analyzer.wave_shape_min_area_px - 80.0)
        if key == ord("6"):
            analyzer.set_wave_shape_params(min_area_px=analyzer.wave_shape_min_area_px + 80.0)
        if key == ord("7"):
            analyzer.set_wave_shape_params(max_area_px=analyzer.wave_shape_max_area_px - 250.0)
        if key == ord("8"):
            analyzer.set_wave_shape_params(max_area_px=analyzer.wave_shape_max_area_px + 250.0)
        if key == ord(","):
        ...
        if key == ord("b"):
            analyzer.blur_mode = (int(analyzer.blur_mode) + 1) % 4
