<Cabbage>
form caption("Wave Synth") size(1260, 1425), guiMode("queue"), pluginId("wSyn"), colour(20, 20, 20)
...
rslider channel("W_freq"), bounds(48, 29, 70, 70), text("Freq"), range(10, 800, 220, 0.3, 0.01), trackerColour(180,120,60)
...
groupbox bounds(5, 1070, 1250, 110), channel("spec36Group"), text("Instr36: FFT Spectrum Synth"), colour(44, 56, 48), fontColour(220, 220, 220), outlineColour(82, 106, 90) {
button bounds(3, 50, 36, 24), channel("inst36_on"), text("On"), value(0), colour:0("#3c4652"), colour:1("#2ecc71"), fontColour("white")

rslider channel("S_basefreq"), bounds(48, 29, 70, 70), text("AdBaseHz"), range(10, 800, 93.75, 0.35, 0.01), trackerColour(110,180,120)
rslider channel("S_amp_dB"), bounds(140, 29, 70, 70), text("AdAmp dB"), range(-96, 6, -18, 1, 0.1), trackerColour(110,180,120)
rslider channel("S_lopass"), bounds(232, 29, 70, 70), text("AdLP Hz"), range(20, 20000, 9000, 0.35, 1), trackerColour(110,180,120)
nslider channel("S_partials"), bounds(324, 38, 80, 24), text("AdParts"), range(1, 512, 256, 1, 1), fontSize(13)
rslider channel("S_sinefreq"), bounds(420, 29, 70, 70), text("SinHz"), range(50, 800, 220, 0.35, 0.01), trackerColour(200,160,90)
rslider channel("S_sineamp_dB"), bounds(512, 29, 70, 70), text("SinAmp dB"), range(-96, 6, -18, 1, 0.1), trackerColour(200,160,90)
combobox bounds(604, 38, 96, 24), channel("S_outmode"), value(1), text("AM","AdOnly")
combobox bounds(712, 38, 120, 24), channel("S_am_ratio"), value(1), text("1:1","1:2","1:3","1:4","1:6","1:8","1:12","1:16","2:1","3:1","4:1")
nslider channel("S_env_attack"), bounds(844, 34, 84, 24), text("Atk s"), range(0.001, 5, 0.01, 0.5, 0.001), fontSize(13)
nslider channel("S_env_release"), bounds(936, 34, 84, 24), text("Rel s"), range(0.005, 10, 0.25, 0.5, 0.001), fontSize(13)
}
...
gihOsc OSCinit 8100
  ...
  ; classic waveforms
    ...
    ; Live center-slit FFT magnitude table (512 bins) + chunk-state table.
    ; Filled from OSC /slit_fft/chunk packets in instr 10.
    giSlitFftAmp ftgen 0, 0, 512, 7, 0, 512, 0
    giSlitFftSeen ftgen 0, 0, 32, 7, 0, 32, 0
    ; Frequency multipliers for spectrum synth bins: 1..512.
    giSlitFftFreq ftgen 0, 0, 512, -7, 1, 512, 512

    ; Route all incoming MIDI notes to instr 36.
    massign 0, 36
...
instr 1
    ...
    if (kboot == 1 || ktrig_layout > 0) then
        ...
        ky_spec36 = ky_wave34 + kh_wave34_vis + 5
        Sspec36 sprintfk "bounds(5, %.0f, 1250, 110)", ky_spec36
        cabbageSet 1, "spec36Group", Sspec36
        kboot = 0
    endif
...
instr 10
    ...
    kSlitFftFrameId init -1
    kSlitFftChunkCount init 32
    kSlitFftReceived init 0
    kfftCentroid init 0
...
read_slit_chunk:
    ...
    ; Live slit FFT magnitude chunks: frame_id, chunk_idx, chunk_count, 16 bins
read_slit_fft_chunk:
    ktr OSClisten gihOsc, "/slit_fft/chunk", "iiiffffffffffffffff", \
        kslFftFrame, kslFftChunk, kslFftCount, \
        kff00, kff01, kff02, kff03, kff04, kff05, kff06, kff07, \
        kff08, kff09, kff10, kff11, kff12, kff13, kff14, kff15
    if (ktr == 1) then
        if (kslFftCount > 0 && kslFftCount <= 32) then
            if (kslFftFrame != kSlitFftFrameId || kslFftCount != kSlitFftChunkCount) then
                kSlitFftFrameId = kslFftFrame
                kSlitFftChunkCount = kslFftCount
                kSlitFftReceived = 0
                kidx = 0
                while (kidx < 32) do
                    tablew 0, kidx, giSlitFftSeen
                    kidx += 1
                od
                chnset 0, "slit_fft_ready"
            endif

            if (kslFftChunk >= 0 && kslFftChunk < kSlitFftChunkCount) then
                kseen table kslFftChunk, giSlitFftSeen
                if (kseen < 0.5) then
                    tablew 1, kslFftChunk, giSlitFftSeen
                    kSlitFftReceived += 1
                endif

                kbase = kslFftChunk * 16
                if (kbase + 15 < 512) then
                    tablew kff00, kbase + 0, giSlitFftAmp
                    tablew kff01, kbase + 1, giSlitFftAmp
                    tablew kff02, kbase + 2, giSlitFftAmp
                    tablew kff03, kbase + 3, giSlitFftAmp
                    tablew kff04, kbase + 4, giSlitFftAmp
                    tablew kff05, kbase + 5, giSlitFftAmp
                    tablew kff06, kbase + 6, giSlitFftAmp
                    tablew kff07, kbase + 7, giSlitFftAmp
                    tablew kff08, kbase + 8, giSlitFftAmp
                    tablew kff09, kbase + 9, giSlitFftAmp
                    tablew kff10, kbase + 10, giSlitFftAmp
                    tablew kff11, kbase + 11, giSlitFftAmp
                    tablew kff12, kbase + 12, giSlitFftAmp
                    tablew kff13, kbase + 13, giSlitFftAmp
                    tablew kff14, kbase + 14, giSlitFftAmp
                    tablew kff15, kbase + 15, giSlitFftAmp
                endif
            endif

            if (kSlitFftReceived >= kSlitFftChunkCount && kSlitFftChunkCount == 32) then
                chnset 1, "slit_fft_ready"
                chnset kSlitFftFrameId, "slit_fft_frame"
            endif
        endif
        kgoto read_slit_fft_chunk
    endif
...
instr 36
    kbase chnget "S_basefreq"
    kamp_dB chnget "S_amp_dB"
    klpf chnget "S_lopass"
    ipartials chnget "S_partials"
    ksinefreq chnget "S_sinefreq"
    ksineamp_dB chnget "S_sineamp_dB"
    koutmode chnget "S_outmode"
    kam_ratio_sel chnget "S_am_ratio"
    kenv_att chnget "S_env_attack"
    kenv_rel chnget "S_env_release"
    kenable chnget "inst36_on"

    ifreq_midi cpsmidi
    ifreq_midi *= 0.5
    iamp_midi_ ampmidi 1
    iamp_midi = 0.6 + iamp_midi_*0.4
    
    imidi_active = (ifreq_midi > 0 ? 1 : 0)
   
    kplayfreq = (imidi_active > 0.5 ? ifreq_midi : ksinefreq)
    kvelamp = (imidi_active > 0.5 ? iamp_midi : 1.0)

    kbase = limit(kbase, 10, 800)
    ksinefreq = limit(ksinefreq, 50, 800)
    kplayfreq = limit(kplayfreq, 10, 20000)
    klpf = limit(klpf, 20, 20000)
    kenv_att = limit(kenv_att, 0.001, 10)
    kenv_rel = limit(kenv_rel, 0.005, 20)
    if (ipartials <= 0) then
        ipartials = 256
    endif
    ipartials = int(limit(ipartials, 1, 512))
    kamp = ampdbfs(kamp_dB) * kvelamp
    ksineamp = ampdbfs(ksineamp_dB) * kvelamp

    kam_ratio = 1
    if (kam_ratio_sel < 1.5) then
        kam_ratio = 1
    elseif (kam_ratio_sel < 2.5) then
        kam_ratio = 0.5
    elseif (kam_ratio_sel < 3.5) then
        kam_ratio = 0.3333333333
    elseif (kam_ratio_sel < 4.5) then
        kam_ratio = 0.25
    elseif (kam_ratio_sel < 5.5) then
        kam_ratio = 0.1666666667
    elseif (kam_ratio_sel < 6.5) then
        kam_ratio = 0.125
    elseif (kam_ratio_sel < 7.5) then
        kam_ratio = 0.0833333333
    elseif (kam_ratio_sel < 8.5) then
        kam_ratio = 0.0625
    elseif (kam_ratio_sel < 9.5) then
        kam_ratio = 2
    elseif (kam_ratio_sel < 10.5) then
        kam_ratio = 3
    else
        kam_ratio = 4
    endif

    kspecbase = (imidi_active > 0.5 ? kplayfreq : kbase)
    if (koutmode < 1.5) then
        kspecbase = limit(kplayfreq * kam_ratio, 10, 20000)
    endif

    ; Additive reconstruction from 512-bin amplitude spectrum.
    ; Frequencies are baseHz * (bin_index + 1) via giSlitFftFreq.
    ; Magnitude-only FFT data does not include phase, so initialize partials
    ; with random phase to avoid pulse-train clicks from full phase alignment.
    aspec adsynt2 kamp, kspecbase, giSine, giSlitFftFreq, giSlitFftAmp, ipartials, 2
    aspec dcblock aspec
    aspec lpf18 aspec, klpf, 0.55, 0.35

    aenv = 1
    if (imidi_active > 0.5) then
        aenv madsr i(kenv_att), 0.01, 1.0, i(kenv_rel)
    endif

    asine oscili ksineamp, kplayfreq, giSine

    if (koutmode < 1.5) then
        aout = asine * (1.0 + (aspec*5))
    else
        aout = aspec
    endif

    aout = aout * aenv * kenable
