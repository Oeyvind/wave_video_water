...
def render_spectrum_overlay(frame, xf, yf, peaks, spatial_freqs=None, show_spectrogram=True, show_summary=True,
    ...
    # Spatial frequency panel (below temporal)
    if spatial_freqs is not None:
        spatial_box_h = 250
        y1 = y0 + box_h + margin
        
        if y1 + spatial_box_h < h:
            ...
            if show_spectrogram:
                ...
                cv2.putText(spatial_panel, f"{max_spatial_freq:.0f}/frame", (pad_l + plot_w - 50, pad_t + plot_h + 16), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (220, 220, 220), 1)
            ...
            roi = frame[y1:y1 + spatial_box_h, x0:x0 + box_w]
            blended = cv2.addWeighted(roi, 1.0 - alpha, spatial_panel, alpha, 0)
            frame[y1:y1 + spatial_box_h, x0:x0 + box_w] = blended
